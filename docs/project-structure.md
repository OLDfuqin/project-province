...
| 文件 | 用途 |
| --- | --- |
| `main.gd` | 主界面协调器；连接节点信号，管理本国订单列表与取消按钮并提交订单。当前还直接汇总军队兵力并以 `floor(兵力/2)` 派生国家维护费，生成维护/负债摘要和月度维护日志文字，并保留道路端点费用与科技折扣的本地估价 fallback，同时刷新固定回合按钮、地图和分阶段报告。端点选定后的权威道路报价和资格来自 bridge。 |
...
`main.gd` 以界面编排为主要职责，但维护费摘要、公式和对应文字仍在其中重复实现，道路界面在尚无 bridge 费用结果时也会用本地端点费用与科技折扣估价；`province_management_window.gd` 还以固定单价4计算可负担征兵人数。这些是当前实现边界和同步负担，不是理想依赖方向。道路端点选定后，权威报价和科技资格来自 `get_road_order_quote()`；本地 `_road_required_level()` 当前没有调用点，不构成生效的资格回退。另一本地估价没有单独处理 `capital`，会落入每端点500的默认值，而核心按每端点300计算，存在展示估价漂移风险。权威扣款、报价、资格与订单执行仍以 C++ 核心及桥接返回结果为准；修改这些规则时必须同步核对对应 GDScript。后续可由桥接层提供只读维护摘要和完整报价/可负担信息，并把纯展示文字下沉到 `game/scripts/ui/`，逐步移除 UI 中的规则副本。
...
这类脚本可以组装只读视图，但不得成为权威状态来源。上节列出的维护、道路本地估价 fallback 和征兵可负担人数是现存的重复显示/校验逻辑，必须与核心同步；最终拒绝或执行结果仍由桥接层和 C++ 核心决定。
