...
func _initialize() -> void:
    ...
    if inspector.current_mode() != "empty" or \
            not inspector.get_node("Body/Empty").visible or \
            inspector.get_node("Body/Content").get_child_count() != 0:
    ...
    if inspector.current_mode() != "province_summary" or \
            ...
            inspector.get_node("Body/Empty").visible or \
            inspector.get_node("Body/Content").get_child_count() != 1 or \
            inspector.get_node("Body/Content").get_child(0) != summary:
    ...
    if inspector.current_mode() != "empty" or \
            not inspector.get_node("Body/Empty").visible or \
            inspector.get_node("Body/Content").get_child_count() != 0 or \
